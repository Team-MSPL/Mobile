import {useNavigation} from '@react-navigation/native';
import {Text, Center, Box, ScrollView, Button, VStack, HStack, Divider, Stack, Wrap} from 'native-base';
import {useState} from 'react';
import {Alert} from 'react-native';
import {useAppDispatch, useAppSelector} from '../../redux';
import {travelSliceActions} from '../../redux/travel-info/travel.slice';
import CustomButton from '../../utill/component/custom-button';
export default function SelectCity() {
	const viewList = [
		{id: 0, title: '서울', sub: [{id: 0, subTitle: '전체'}]},
		{id: 1, title: '부산', sub: [{id: 0, subTitle: '전체'}]},
		{id: 2, title: '대구', sub: [{id: 0, subTitle: '전체'}]},
		{id: 3, title: '인천', sub: [{id: 0, subTitle: '전체'}]},
		{id: 4, title: '광주', sub: [{id: 0, subTitle: '전체'}]},
		{id: 5, title: '대전', sub: [{id: 0, subTitle: '전체'}]},
		{id: 6, title: '울산', sub: [{id: 0, subTitle: '전체'}]},
		{
			id: 7,
			title: '경기',
			sub: [
				{id: 0, subTitle: '전체'},
				{id: 1, subTitle: '수원시'},
				{id: 2, subTitle: '고양시'},
				{id: 3, subTitle: '성남시'},
				{id: 4, subTitle: '용인시'},
				{id: 5, subTitle: '부천시'},
				{id: 6, subTitle: '안산시'},
				{id: 7, subTitle: '남양주시'},
				{id: 8, subTitle: '안양시'},
				{id: 9, subTitle: '화성시'},
				{id: 10, subTitle: '평택시'},
				{id: 11, subTitle: '의정부시'},
				{id: 12, subTitle: '시흥시'},
				{id: 13, subTitle: '파주시'},
				{id: 14, subTitle: '김포시'},
				{id: 15, subTitle: '광명시'},
				{id: 16, subTitle: '광주시'},
				{id: 17, subTitle: '군포시'},
				{id: 18, subTitle: '오산시'},
				{id: 19, subTitle: '이천시'},
				{id: 20, subTitle: '양주시'},
				{id: 21, subTitle: '안성시'},
				{id: 22, subTitle: '구리시'},
				{id: 23, subTitle: '포천시'},
				{id: 24, subTitle: '의왕시'},
				{id: 25, subTitle: '하남시'},
				{id: 26, subTitle: '여주시'},
				{id: 27, subTitle: '양평군'},
				{id: 28, subTitle: '동두천시'},
				{id: 29, subTitle: '과천시'},
				{id: 30, subTitle: '가평군'},
				{id: 31, subTitle: '연천군'},
			],
		},
		{
			id: 8,
			title: '강원',
			sub: [
				{id: 1, subTitle: '전체'},
				{id: 2, subTitle: '강릉시'},
				{id: 3, subTitle: '동해시'},
				{id: 4, subTitle: '삼척시'},
				{id: 5, subTitle: '속초시'},
				{id: 6, subTitle: '원주시'},
				{id: 7, subTitle: '춘천시'},
				{id: 8, subTitle: '태백시'},
				{id: 9, subTitle: '고성군'},
				{id: 10, subTitle: '양구군'},
				{id: 11, subTitle: '양양군'},
				{id: 12, subTitle: '영월군'},
				{id: 13, subTitle: '인제군'},
				{id: 14, subTitle: '정선군'},
				{id: 15, subTitle: '철원군'},
				{id: 16, subTitle: '평창군'},
				{id: 17, subTitle: '홍천군'},
				{id: 18, subTitle: '화천군'},
				{id: 19, subTitle: '횡성군'},
			],
		},
		{
			id: 9,
			title: '충북',
			sub: [
				{id: 0, subTitle: '전체'},
				{id: 1, subTitle: '제천시'},
				{id: 2, subTitle: '청주시'},
				{id: 3, subTitle: '충주시'},
				{id: 4, subTitle: '괴산군'},
				{id: 5, subTitle: '단양군'},
				{id: 6, subTitle: '보은군'},
				{id: 7, subTitle: '영동군'},
				{id: 8, subTitle: '옥천군'},
				{id: 9, subTitle: '음성군'},
				{id: 10, subTitle: '진천군'},
				{id: 11, subTitle: '증평군'},
				{id: 12, subTitle: '청원군'},
			],
		},
		{
			id: 10,
			title: '충남',
			sub: [
				{id: 0, subTitle: '전체'},
				{id: 1, subTitle: '공주시'},
				{id: 2, subTitle: '논산시'},
				{id: 3, subTitle: '보령시'},
				{id: 4, subTitle: '서산시'},
				{id: 5, subTitle: '아산시'},
				{id: 6, subTitle: '천안시'},
				{id: 7, subTitle: '금산군'},
				{id: 8, subTitle: '당진시'},
				{id: 9, subTitle: '부여군'},
				{id: 10, subTitle: '서천군'},
				{id: 11, subTitle: '예산군'},
				{id: 12, subTitle: '연기군'},
				{id: 13, subTitle: '청양군'},
				{id: 14, subTitle: '태안군'},
				{id: 15, subTitle: '홍성군'},
			],
		},
		{
			id: 11,
			title: '전북',
			sub: [
				{id: 1, subTitle: '전체'},
				{id: 2, subTitle: '군산시'},
				{id: 3, subTitle: '김제시'},
				{id: 4, subTitle: '남원시'},
				{id: 5, subTitle: '익산시'},
				{id: 6, subTitle: '전주시'},
				{id: 7, subTitle: '정읍시'},
				{id: 8, subTitle: '고창군'},
				{id: 9, subTitle: '무주군'},
				{id: 10, subTitle: '부안군'},
				{id: 11, subTitle: '순창군'},
				{id: 12, subTitle: '완주군'},
				{id: 13, subTitle: '임실군'},
				{id: 14, subTitle: '장수군'},
				{id: 15, subTitle: '진안군'},
			],
		},
		{
			id: 13,
			title: '전남',
			sub: [
				{id: 1, subTitle: '광양시'},
				{id: 2, subTitle: '나주시'},
				{id: 3, subTitle: '목포시'},
				{id: 4, subTitle: '순천시'},
				{id: 5, subTitle: '여수시'},
				{id: 6, subTitle: '강진군'},
				{id: 7, subTitle: '고흥군'},
				{id: 8, subTitle: '곡성군'},
				{id: 9, subTitle: '구례군'},
				{id: 10, subTitle: '담양군'},
				{id: 11, subTitle: '무안군'},
				{id: 12, subTitle: '보성군'},
				{id: 13, subTitle: '신안군'},
				{id: 14, subTitle: '영광군'},
				{id: 15, subTitle: '영암군'},
				{id: 16, subTitle: '완도군'},
				{id: 17, subTitle: '장성군'},
				{id: 18, subTitle: '장흥군'},
				{id: 19, subTitle: '진도군'},
				{id: 20, subTitle: '함평군'},
				{id: 21, subTitle: '해남군'},
				{id: 22, subTitle: '화순군'},
			],
		},
		{
			id: 14,
			title: '경북',
			sub: [
				{id: 0, subTitle: '전체'},
				{id: 1, subTitle: '경산시'},
				{id: 2, subTitle: '경주시'},
				{id: 3, subTitle: '구미시'},
				{id: 4, subTitle: '김천시'},
				{id: 5, subTitle: '문경시'},
				{id: 6, subTitle: '상주시'},
				{id: 7, subTitle: '안동시'},
				{id: 8, subTitle: '영주시'},
				{id: 9, subTitle: '영천시'},
				{id: 10, subTitle: '포항시'},
				{id: 11, subTitle: '고령군'},
				{id: 12, subTitle: '군위군'},
				{id: 13, subTitle: '봉화군'},
				{id: 14, subTitle: '성주군'},
				{id: 15, subTitle: '영양군'},
				{id: 16, subTitle: '영덕군'},
				{id: 17, subTitle: '예천군'},
				{id: 18, subTitle: '울릉군'},
				{id: 19, subTitle: '울진군'},
				{id: 20, subTitle: '의성군'},
				{id: 21, subTitle: '청도군'},
				{id: 22, subTitle: '청송군'},
				{id: 23, subTitle: '칠곡군'},
			],
		},
		{
			id: 15,
			title: '경남',
			sub: [
				{id: 1, subTitle: '전체'},
				{id: 2, subTitle: '거제시'},
				{id: 3, subTitle: '김해시'},
				{id: 4, subTitle: '밀양시'},
				{id: 5, subTitle: '사천시'},
				{id: 6, subTitle: '양산시'},
				{id: 7, subTitle: '진주시'},
				{id: 8, subTitle: '창원시'},
				{id: 9, subTitle: '통영시'},
				{id: 10, subTitle: '거창군'},
				{id: 11, subTitle: '고성군'},
				{id: 12, subTitle: '남해군'},
				{id: 13, subTitle: '산청군'},
				{id: 14, subTitle: '의령군'},
				{id: 15, subTitle: '창녕군'},
				{id: 16, subTitle: '하동군'},
				{id: 17, subTitle: '함안군'},
				{id: 18, subTitle: '함양군'},
				{id: 19, subTitle: '합천군'},
			],
		},
	];
	const [select, setSelect] = useState(0);
	const navigation = useNavigation();
	const {region} = useAppSelector(state => state.travelSlice);
	const dispatch = useAppDispatch();

	const selectRegion = (e: string) => {
		//viewList[select].title+' '+    이거 파싱해야되는데 실화냐 ㅋ
		if (e === '전체' || region.includes('전체')) {
			dispatch(travelSliceActions.selectRegion([e]));
		} else if (region.includes(e)) {
			let copy = [...region];
			copy = copy.filter(item => item !== e);
			dispatch(travelSliceActions.selectRegion(copy));
		} else {
			let copy = [...region];
			copy.push(e);
			dispatch(travelSliceActions.selectRegion(copy));
		}
	};
	const deleteRegion = (e: string) => {
		let copy = [...region];
		copy = region.filter(item => item != e);
		dispatch(travelSliceActions.selectRegion(copy));
	};
	const selectCity = (e: number) => {
		if (region) {
			dispatch(travelSliceActions.setRegion());
		}
		setSelect(e);
	};
	const goNext = () => {
		navigation.navigate('SelectDay');
	};
	return (
		<Box bgColor='#EFFBFB' p='2'>
			{/* 스테퍼 넣기 */}
			<VStack>
				<Text fontSize='2xl' bold color='black'>
					여행 일정 등록
				</Text>
				<Text fontSize='md' color='grey'>
					여행 지역을 알려주세요.
				</Text>
			</VStack>
			<Divider my='5' />
			<VStack space='10'>
				<HStack>
					<Box w='1/2' h='300'>
						<Text fontSize='xl' color='grey'>
							지역
						</Text>
						<Box h='full' borderWidth='1px' borderColor='grey'>
							<ScrollView>
								{viewList.map((item, idx) => {
									return (
										<Button
											key={idx}
											bgColor={select == item.id ? '#ABD9FF' : '#EFFBFB'}
											justifyContent='start'
											onPress={() => selectCity(item.id)}>
											<Text color={select == item.id ? 'blue.500' : 'black'} bold>
												{item.title}
											</Text>
										</Button>
									);
								})}
							</ScrollView>
						</Box>
					</Box>
					<Box w='1/2' h='300'>
						<Text fontSize='xl' color='grey'>
							상세 지역
						</Text>
						<Box h='full' borderWidth='1px' borderColor='grey'>
							<ScrollView>
								{viewList[select].sub.map((item, idx) => {
									return (
										<Button
											key={idx}
											bgColor={region.includes(item.subTitle) ? '#ABD9FF' : '#EFFBFB'}
											colorScheme='black'
											justifyContent='start'
											onPress={() => {
												selectRegion(item.subTitle);
											}}>
											<Text color={region.includes(item.subTitle) ? 'blue.500' : 'black'} bold>
												{item.subTitle}
											</Text>
										</Button>
									);
								})}
							</ScrollView>
						</Box>
					</Box>
				</HStack>
				<Box w='full' h='20' bgColor='#B0B0B0'>
					<Text color='white' bold ml='3'>
						{region.length}개
					</Text>
					<ScrollView flexDir='row' horizontal>
						{region.map((item, idx) => {
							return (
								<Button
									m='2'
									key={idx}
									bgColor='white'
									borderRadius='20px'
									borderWidth='1px'
									borderColor='blue.500'
									onPress={() => {
										deleteRegion(item);
									}}>
									<Text color='blue.500'>{item}</Text>
								</Button>
							);
						})}
					</ScrollView>
				</Box>
				<CustomButton label='다음단계' onPress={goNext} isDisabled={!region.length} />
			</VStack>
		</Box>
	);
}
