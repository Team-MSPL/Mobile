...
export default function SearchRecommend({navigation, route}: any) {
	...
	const addPlace = () => {
		let copy = [...timetable];
		let copy2 = [...copy[route.params.index]];
		console.log('before', copy2);
		placeState && (copy2[route.params.idx] = placeState);
		if (route.params.category == '숙소') {
			let updateItem = {
				...placeState,
				x: copy[route.params.index + 1][0].x,
				y: copy[route.params.index + 1][0].y,
				takenTime: copy[route.params.index + 1][0].takenTime,
			};
	...
	return (
		<SearchPlaceContainer
			...
			<SearchContainer height={heightPercentage(497)}>
				<GooglePlacesAutocomplete
					...
					onPress={async (data, details) => {
						...
						const datas = {
							...timetable[route.params.index][route.params.idx],
							name: details?.name,
			...
			{placeState ? (
				<BottomContainer height={heightPercentage(182)}>
