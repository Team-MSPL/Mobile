...
import {formatPrice, makeCalendarData, WEEKDAYS} from '../../utill/component/product/reservation-calander';
import {
	safeNum,
	lowestPriceFromEntry,
	mergeSkuCalendars,
	formatCompactPrice,
	buildMonthMatrixLocal,
} from '../../utill/component/product/reservation-function';
import {useRoute} from '@react-navigation/native';
import {useAppDispatch, useAppSelector} from '../../redux';
import {getQueryPackage, travelSliceActions} from '../../redux/travel-info/travel.slice';
import {PretendardSemiBoldText} from '../../utill/layout/layout';
...
function ProductReservation({navigation}: any) {
	const route = useRoute();
	const params = route.params;
	const {s_date} = useAppSelector(state => state.travelSlice);
	...
	const [currentMonth, setCurrentMonth] = useState(() => {
		...
		if (selectedSkuFromParams && typeof selectedSkuFromParams === 'object') {
			...
			const start =
				bounds.min ??
				(Array.isArray(params?.online_s_date) ? params.online_s_date[0] : params.online_s_date) ??
				s_date;
		...
		async function load() {
			...
			try {
				...
				const calendar_detail =
					firstSku?.calendar_detail ??
					firstSku?.calendar ??
					firstItem?.calendar_detail ??
	...
	const calInfo = useMemo(() => {
		...
		const calendar_detail_pkg =
			pkgData?.calendar_detail_merged && Object.keys(pkgData.calendar_detail_merged).length > 0
		...
		const sale_s_date_pkg = Array.isArray(firstItem?.sale_s_date)
			? firstItem.sale_s_date[0]
			: firstItem?.sale_s_date;
		const sale_e_date_pkg = Array.isArray(firstItem?.sale_e_date)
			? firstItem.sale_e_date[0]
			: firstItem?.sale_e_date;
	...
	const prevDisabled = earliestMonth
		? currentMonth.year() === earliestMonth.year() && currentMonth.month() === earliestMonth.month()
		: false;
	const nextDisabled = latestMonth
	...
	function getPriceFromEntry(entry: any, time?: string | null) {
		...
		const discountAmount =
			original !== undefined && display !== undefined && original > display ? Math.floor(original - display) : 0;
	...
	const handleDayPress = (cell: any) => {
		...
		const hasPrice =
			(timeOpts && timeOpts.length > 0) ||
			(priceInfo && priceInfo.display !== undefined && priceInfo.display !== null);
		...
		setPendingDate(null);
		dispatch(travelSliceActions.updateFiled({field: 's_date', value: dateStr}));
		dispatch(travelSliceActions.updateFiled({field: 'e_date', value: dateStr}));
	};
	...
	const handleChooseTime = (time: string | null) => {
		...
		if (dateSetting === '02') {
			...
			dispatch(travelSliceActions.updateFiled({field: 's_date', value: pendingDate}));
			dispatch(travelSliceActions.updateFiled({field: 'e_date', value: pendingDate}));
		} else {
	...
	const goNext = async () => {
		if (dateSetting === '02') {
			if (!isRangeValid()) {
				await logEvent(`goCalendarErrorDay`, {pkgName: pkgData?.pkg_name});
				Alert.alert(
					'날짜 선택',
					`선택 범위가 유효하지 않습니다. 최소 ${minDays}일, 최대 ${
						isFinite(maxDays) ? maxDays : '제한 없음'
					}일 사이로 선택하세요.`,
				);
				return;
			}
			...
			dispatch(travelSliceActions.updateFiled({field: 's_date', value: s}));
			dispatch(travelSliceActions.updateFiled({field: 'e_date', value: e}));
			...
			const outgoingDisplay =
				filledSource === 'display_price' && fallbackDisplay !== undefined
					? fallbackDisplay
		...
		<View style={{flex: 1, backgroundColor: '#fff'}}>
			<View style={{alignItems: 'center', marginTop: 60, marginBottom: 20}}>
				<View style={{flexDirection: 'row', alignItems: 'center', marginTop: 12}}>
					<TouchableOpacity
						onPress={() => {
							if (!prevDisabled) setCurrentMonth(currentMonth.subtract(1, 'month'));
						}}
						disabled={prevDisabled}
						style={{marginHorizontal: 16}}>
						<Text style={{fontSize: 24, color: prevDisabled ? colors.grey200 : colors.grey800}}>{'<'}</Text>
					</TouchableOpacity>
					<Text style={{fontSize: 19, fontWeight: 'bold', letterSpacing: -1, color: colors.Black}}>
						{monthLabel}
					</Text>
					<TouchableOpacity
						onPress={() => {
							if (!nextDisabled) setCurrentMonth(currentMonth.add(1, 'month'));
						}}
				...
				<Text style={{color: colors.PointYellow, fontSize: 16, marginHorizontal: 8}}>최저가/할인중</Text>
			</View>
							...
							{week.map((cell: any, j: number) => {
								...
								if (cell.soldOut || !selectable) {
									return (
										<View
											...
											<Text style={{color: colors.grey300, fontSize: 12}}>
												{cell.soldOut ? '매진' : '-'}
											</Text>
										</View>
									);
								...
								const isSel =
									dateSetting === '02'
										? Boolean(
												rangeStart &&
													rangeEnd &&
													isDateInRange(cell.date, rangeStart, rangeEnd),
										  )
										: selected === cell.date;
								...
								return (
									<TouchableOpacity
										...
										<Text
											style={{
												color: isSel ? '#fff' : colors.PointYellow,
												fontWeight: isSel ? 'bold' : 'normal',
												fontSize: 11,
												marginTop: 2,
											}}>
											{cell.price ? formatCompactPrice(cell.price) : ''}
										</Text>
									</TouchableOpacity>
								);
							})}
			...
			<View style={{padding: 24, backgroundColor: '#fff'}}>
				<Button height={58} disabled={nextButtonDisabled} onPress={goNext}>
					<Text style={{color: colors.backgroundWhite, fontSize: 20, fontWeight: '500'}}>다음으로</Text>
				</Button>
			</View>
			...
			<Modal
				...
				<View style={modalStyles.backdrop}>
					<View style={modalStyles.container}>
						<Text style={{marginBottom: 12, color: colors.Black}}>시간을 선택하세요</Text>
							...
							{timeOptions.map(opt => {
								...
								return (
									<TouchableOpacity
										...
										<Text
											style={
												active ? {color: '#fff', fontWeight: 'bold'} : {color: colors.grey600}
											}>
											{opt.time}
										</Text>
										<Text style={active ? {color: '#fff'} : {color: colors.grey600}}>
						...
						<View style={{flexDirection: 'row', justifyContent: 'flex-end', gap: 8}}>
							<View style={{marginRight: 8}}>
								<Button
									...
									<Text style={{color: colors.backgroundWhite, fontSize: 15}}>취소</Text>
								</Button>
							</View>
							<Button
								height={38}
								...
								<Text style={{color: colors.backgroundWhite, fontSize: 15}}>확인</Text>
							</Button>
						</View>
					</View>
				</View>
