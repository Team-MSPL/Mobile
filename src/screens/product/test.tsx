...
import {View, FlatList, TouchableOpacity, Dimensions, Image, Alert, ScrollView, Pressable} from 'react-native';
...
import {parseKkdayCategoryKorean} from '../../utill/kkday/kkdayCategoryToKorean';
import {
	getRefundTag,
	...
	getPriceInfo,
	earliestBookingText,
} from '../../utill/component/product/good-product-function';
import Pdml from '../../utill/component/pdml'; // reuse PDML components as detail
...
export default function ProductGoodProduct({navigation}: any) {
	...
	useEffect(() => {
		async function fetchProductDetail() {
			...
			setLoading(true);
			try {
				const res = await dispatch(getQueryProduct(params.product?.prod_no ?? params.prod_no)).unwrap();
				console.log('zz', res);
				const resData = res ?? {};
				console.log(resData);
				...
				if (resData && resData.prod && resData.pkg) {
					...
					const firstAvailable =
						resData.pkg.find((p: any) => !!p.sale_s_date && !!p.sale_e_date) ?? resData.pkg[0];
	...
	// Derived values
	const discountPrice =
		typeof product?.b2b_min_price === 'number' ? product.b2b_min_price : Number(product?.b2b_min_price || 0);
	const originalPrice =
		typeof product?.b2c_min_price === 'number' ? product.b2c_min_price : Number(product?.b2c_min_price || 0);
	...
	const images =
		Array.isArray(product?.img_list) && product.img_list.length > 0
			? product.img_list
	...
	// 		const isOnlyTicketSpec =
	// 			Array.isArray(specs) &&
	// 			specs.length === 1 &&
	// 			typeof specs[0]?.spec_title === 'string' &&
	// 			specs[0].spec_title.trim().toLowerCase() === '티켓 종류';
	...
	// 		const msg =
	// 			err?.response?.data?.result_msg ?? err?.message ?? '패키지 정보를 불러오는 중 오류가 발생했습니다.';
	// 		Alert.alert('오류', msg);
	// 	} finally {
	// 		setReservationLoading(false);
	// 	}
			...
			{images.map((_, idx) => (
				<View
					...
					style={{
						...
						height: 8,
						borderRadius: 4,
						backgroundColor: idx === imgIndex ? colors.Blue2 : colors.Gray200,
						marginHorizontal: 4,
					}}
				/>
			))}
		...
		<>
			<ScrollView style={{flex: 1, backgroundColor: '#fff'}}>
					...
					<FlatList
						...
						renderItem={({item}) => (
							<View style={{width: SCREEN_WIDTH, height: 291}}>
								<Image source={{uri: item}} style={{width: '100%', height: 291}} resizeMode='cover' />
							</View>
						)}
				...
				<View style={{paddingHorizontal: 24, paddingTop: 32, paddingBottom: 8}}>
					...
					<View style={{flexDirection: 'row', flexWrap: 'wrap', marginBottom: 24, marginTop: 24}}>
						{categoryList.length > 0 &&
							categoryList.map((cat, idx) => (
								<View style={{backgroundColor: colors.gray100, borderRadius: 8, padding: 4}}>
									<PretendardSemiBoldText
										size={16}
										lineHeight={22}
										numberOfLines={1}
				...
				<View style={{backgroundColor: colors.gray100, height: 18, width: '100%'}} />
				<View style={{padding: 24}}>
					...
					{pkgList.map((pkg, idx) => {
						...
						return (
							<TouchableOpacity
								key={pkg.pkg_no}
								onPress={() => {
									if (!isSoldOut) setSelectedPkgNo(pkg.pkg_no);
								}}
								style={{
									borderWidth: 1,
									borderColor: isSoldOut ? colors.Gray2 : isSelected ? colors.Blue2 : colors.Gray2,
									borderRadius: 12,
									backgroundColor: isSoldOut ? '#fff' : isSelected ? colors.Blue1 : '#fafbfc',
									marginBottom: 16,
									padding: 18,
										...
										{getPriceInfo(pkg).hasDiscount ? (
											<>
												...
												<PretendardSemiBoldText
													...
													numberOfLines={1}
													color={colors.Black}>
													{formatPrice(getPriceInfo(pkg).display)}원
												</PretendardSemiBoldText>
											</>
										) : (
											<PretendardSemiBoldText
				...
				<View style={{height: 12, backgroundColor: colors.gray100, width: '100%'}} />
			...
			<ButtonContainer>
				<RouteButton
					...
					nextTitle='RecommendSelectWho'
					nextText={reservationLoading ? '로딩 중...' : '예약하기'}></RouteButton>
			</ButtonContainer>
		</>
	);
}
