...
import axiosAuth from '../../redux/api/api';
import {API_ROUTE_RELEASE} from '@env';
...
export function useBookingApi() {
	...
	function buildPayloadForSku(payload: AnyObject, skuId: string): AnyObject {
		...
		if (Array.isArray(payload?.skus)) {
			copy.skus = payload.skus.filter(
				(s: any) => String(s?.sku_id ?? s?.spec_token ?? s?.id ?? '') === String(skuId),
			);
		}

		if (Array.isArray(payload?.traffics)) {
			copy.traffics = payload.traffics.filter(
				(t: any) => String(t?.spec_token ?? t?.sku_id ?? '') === String(skuId),
			);
		}
	...
	const run = useCallback(
		async (payload: AnyObject): Promise<RunResult> => {
			...
			async function callSaveApi(saveDoc: AnyObject, isActiveFlag: boolean) {
				const saveUrl = `${API_ROUTE_RELEASE}/bookingProduct/save`;
				...
				try {
					...
					if (saveErr?.response) {
						console.error(
							'[useBookingApi] Save API error response:',
							saveErr.response.status,
							saveErr.response.data,
						);
			...
			async function bookingAndSaveOnce(subPayload: AnyObject): Promise<SingleResult> {
				...
				const bookingUrl = `${API_ROUTE_RELEASE}/kkday/Booking/`;
				...
				try {
					...
					if (axiosErr?.response) {
						...
						console.debug(
							'[useBookingApi] Booking axios threw and no response - calling Save API with isActive=false. Minimal saveDoc:',
							saveDocMinimal,
						);
				...
				const resp =
					bookingJson && typeof bookingJson === 'object' && bookingJson.data ? bookingJson.data : bookingJson;
				...
				const buyer =
					subPayload?.buyer && typeof subPayload.buyer === 'object'
						? {
								...
								tel_country_code:
									subPayload.buyer.tel_country_code ?? subPayload.buyer.telCountryCode ?? '',
				...
				const skus =
					Array.isArray(subPayload?.skus) && subPayload.skus.length > 0
						...
						: [
								{
									sku_id: subPayload?.item_no
										? String(subPayload.item_no)
										: subPayload?.pkg_no
										? String(subPayload.pkg_no)
										: '',
									qty: subPayload?.qty
										? Number(subPayload.qty)
										: subPayload?.adult_qty
										? Number(subPayload.adult_qty)
										: 1,
									price: subPayload?.total
										? Number(subPayload.total)
										: subPayload?.display_price
										? Number(subPayload.display_price)
										: 0,
								},
						  ];
				...
				const saveDoc: AnyObject = {
					...
					prod_no: Number(
						subPayload?.prod_no ??
							subPayload?.prod_no_local ??
							subPayload?.product?.prod_no ??
							subPayload?.pdt?.prod_no ??
							0,
					),
					pkg_no: subPayload?.pkg_no
						? Number(subPayload.pkg_no)
						: subPayload?.pkg_no_local
						? Number(subPayload.pkg_no_local)
						: undefined,
					...
					buyer,
					s_date: subPayload?.s_date
						? new Date(subPayload.s_date)
						: subPayload?.start_date
						? new Date(subPayload.start_date)
						: subPayload?.s_date_obj
						? new Date(subPayload.s_date_obj)
						: undefined,
					e_date: subPayload?.e_date
						? new Date(subPayload.e_date)
						: subPayload?.end_date
						? new Date(subPayload.end_date)
						: undefined,
