...
// import {getCustomArray, getRawTrafficArray} from '../../../../redux/product/bookingSlice';
// const {
// 	buyer_first_name,
// 	buyer_last_name,
// 	buyer_Email,
...
// const arr = useAppSelector(state => getCustomArray(state.bookingSlice));
// const rawTraffic = useAppSelector(state => getRawTrafficArray(state.bookingSlice));
// /* 1) util: isFilled */
...
// export const readCustomValue = (rawFields: any, cusType: string, fieldId: string) => {
// 	// direct map
// 	const group = customMap?.[cusType];
...
// 	const variants = [
// 		fieldId,
// 		fieldId.replace(/_([a-z])/g, (m: string, p1: string) => p1.toUpperCase()),
// 		fieldId.replace(/_/g, ''),
// 	];
// 	if (group) {
...
// export const findStoredTrafficEntry = (rawFields: any, trafficType: string, specIndex?: number) => {
// 	if (!Array.isArray(rawTraffic)) return undefined;

// 	// 1) exact by spec_index
// 	if (typeof specIndex === 'number') {
// 		const exact = rawTraffic.find(
// 			(r: any) => String(r?.traffic_type) === String(trafficType) && Number(r?.spec_index) === Number(specIndex),
// 		);
...
// export const getRequiredCustomFields = (rawFields: any, cusType: string) => {
...
// export const getRequiredTrafficEntries = (rawFields: any, typesArr: string[]) => {
...
// export const validateCustomSection = (rawFields: any, cusType: string) => {
...
// export const validateTrafficSection = (rawFields: any, typesArr: string[]) => {
// 	const missing: string[] = [];
...
// 		let match = rawTraffic.find(
// 			(r: any) =>
// 				String(r?.traffic_type) === t &&
// 				(typeof req.spec_index === 'number' ? Number(r?.spec_index) === Number(req.spec_index) : true),
// 		);
...
// export const validateSectionBuilt = (rawFields: any, requiredMap: Record<number, Array<any>>) => {
// 	const missing: string[] = [];
...
// 			} else {
// 				const val = req.key === 'guide_lang' ? guideLangCode : [req.key];
// 				if (!isFilled(val)) localMissing.push(req.label || req.key);
// 			}
// 		});
// 		return localMissing;
...
//  */
// export const validateSection = (rawFields: any, sectionIndex: number, options: any): string[] => {
// 	const miss: string[] = [];
// 	const {
// 		hasCus01,
// 		hasCus02,
...
// 	switch (sectionIndex) {
// 		case 1: {
// 			if (!isFilled(buyer_last_name)) miss.push('구매자 성 (Last name)');
// 			if (!isFilled(buyer_first_name)) miss.push('구매자 이름 (First name)');
// 			if (!isFilled(buyer_Email) && !isFilled(buyer_Email)) miss.push('이메일');
// 			if (!isFilled(buyer_tel_number)) miss.push('전화번호');
// 			if (!isFilled(buyer_country)) miss.push('국가 코드');
// 			break;
// 		}
// 		case 2: {
// 			if (rawFields?.guide_lang) {
// 				const isReq = String(rawFields.guide_lang?.is_require ?? 'true').toLowerCase() === 'true';
// 				if (isReq && !isFilled(guideLangCode)) {
// 					miss.push('가이드 언어 선택');
// 				}
// 			}
// 			break;
...
/**
 * validationHelpers.ts
 * Hook-based version (fixed useAppSelector placement)
 */
...
/**
 * Hook: useValidationHelpers
 * Provides all validation utilities that rely on Redux store values.
 */
export const useValidationHelpers = () => {
	// ✅ All Redux selectors must be inside the hook
	const {
		buyer_first_name,
		buyer_last_name,
		buyer_Email,
	...
	/* 2) readCustomValue */
	const readCustomValue = (rawFields: any, cusType: string, fieldId: string) => {
		const group = customMap?.[cusType];
		if (group && Object.prototype.hasOwnProperty.call(group, fieldId)) return group[fieldId];
	...
	/* 3) findStoredTrafficEntry */
	const findStoredTrafficEntry = (rawFields: any, trafficType: string, specIndex?: number) => {
		if (!Array.isArray(rawTraffic)) return undefined;
		...
		if (typeof specIndex === 'number') {
			const exact = rawTraffic.find(
				(r: any) =>
					String(r?.traffic_type) === String(trafficType) && Number(r?.spec_index) === Number(specIndex),
			);
			if (exact) return exact;
		}
	...
	/* 4) getRequiredCustomFields */
	const getRequiredCustomFields = (rawFields: any, cusType: string) => {
		if (!rawFields?.custom) return [];
		const fields: string[] = [];
		Object.entries(rawFields.custom).forEach(([fieldId, specObj]: any) => {
			const useArr = specObj?.use ?? [];
	...
	/* 5) getRequiredTrafficEntries */
	const getRequiredTrafficEntries = (rawFields: any, typesArr: string[]) => {
		const required: Array<{traffic_type: string; spec_index?: number; fieldId: string}> = [];
		if (!Array.isArray(rawFields?.traffics)) return required;
	...
	/* 6) validateCustomSection */
	const validateCustomSection = (rawFields: any, cusType: string) => {
		const missing: string[] = [];
		const reqFields = getRequiredCustomFields(rawFields, cusType);
		if (reqFields.length === 0) return missing;
	...
	/* 7) validateTrafficSection */
	const validateTrafficSection = (rawFields: any, typesArr: string[]) => {
		const missing: string[] = [];
		const requiredEntries = getRequiredTrafficEntries(rawFields, typesArr);
		if (requiredEntries.length === 0) return missing;
		if (!Array.isArray(rawTraffic)) return missing;
		...
		requiredEntries.forEach(req => {
			...
			const foundVal = match ? match[req.fieldId] : undefined;
			if (!isFilled(foundVal)) missing.push(`${t} - ${req.fieldId}`);
		});

		return missing;
	};

	const validateSectionBuilt = (rawFields: any, requiredMap: Record<number, Array<any>>) => {
		const missing: string[] = [];
		const listForSection = (idx: number) => requiredMap[idx] ?? [];
	...
	/* 8) validateSection */
	const validateSection = (rawFields: any, sectionIndex: number, options: any): string[] => {
		const miss: string[] = [];
		const {
			hasCus01,
			hasCus02,
		...
		switch (sectionIndex) {
			case 1: {
				if (!isFilled(buyer_last_name)) miss.push('구매자 성 (Last name)');
				if (!isFilled(buyer_first_name)) miss.push('구매자 이름 (First name)');
				if (!isFilled(buyer_Email)) miss.push('이메일');
				if (!isFilled(buyer_tel_number)) miss.push('전화번호');
				if (!isFilled(buyer_country)) miss.push('국가 코드');
				break;
			}
	...
	return {
		isFilled,
		readCustomValue,
		findStoredTrafficEntry,
		getRequiredCustomFields,
		getRequiredTrafficEntries,
		validateCustomSection,
		validateTrafficSection,
		validateSection,
		validateSectionBuilt,
	};
};
