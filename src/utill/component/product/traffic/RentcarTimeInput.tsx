...
function normalize(t: string) {
	...
	if (parts.length === 1 && parts[0].length > 2) return parts[0].slice(0, 2) + ':' + parts[0].slice(2, 4);
	return cleaned;
}

export default function RentcarTimeInput({
	trafficType,
	field,
	rawFields,
	specIndex,
	...
	const hasHourMin =
		...
		listOptions.some(o => typeof o?.hour !== 'undefined' && typeof o?.min !== 'undefined');
	...
	useEffect(() => {
		...
		onValueChange?.(value ?? ''); /* eslint-disable-next-line react-hooks/exhaustive-deps */
	}, [value, trafficType]);
							...
							{options.map((opt, idx) => {
								...
								return (
									<TouchableOpacity
										...
										<Text style={active ? {color: '#fff'} : {color: 'black'}}>{labelText}</Text>
									</TouchableOpacity>
								);
							})}
						</ScrollView>
	...
	if (hasHourMin) {
		...
		const display = (opt: any) =>
			`${String(opt.hour ?? '').padStart(2, '0')}:${String(opt.min ?? '').padStart(2, '0')}`;
		return (
			<View style={{marginBottom: 12}}>
				<Text style={{marginBottom: 6, color: colors.grey800, fontSize: 24}}>
					{label ?? (field === 's_time' ? '픽업' : '하차')}{' '}
