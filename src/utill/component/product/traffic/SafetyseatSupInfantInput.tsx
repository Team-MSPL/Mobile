...
export default function SafetyseatSupInfantInput({
	trafficType,
	label = '유아 보조석(대여) 수(인펀트)',
	placeholder = '0',
	required = false,
	onValueChange,
}: Props) {
	...
	const stored =
		trafficArray?.find(it => String(it?.traffic_type) === String(trafficType))?.safetyseat_sup_infant ?? '';
	...
	return (
		<View style={{marginBottom: 12}}>
			...
			<TextInput
				...
				accessibilityLabel={`safetyseat-sup-infant-${trafficType}`}
			/>
		</View>
	);
}
