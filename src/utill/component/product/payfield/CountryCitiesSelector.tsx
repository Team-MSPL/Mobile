...
export default function CountryCitiesSelector({
	...
	const stored = customMap?.[cusType]?.country_cities ?? '';
	...
	useEffect(() => {
		...
		const found = (options || []).find(
			c => Array.isArray(c.cities) && c.cities.some(ct => String(ct.city_code) === String(stored)),
		);
		if (found) {
	...
	useEffect(() => {
		if (selectedCityCode !== null && selectedCityCode !== '') {
			dispatch(setCustomField({cusType, fieldId: 'country_cities', value: String(selectedCityCode)}));
			onValueChange?.(selectedCityCode);
		} else {
			dispatch(setCustomField({cusType, fieldId: 'country_cities', value: ''}));
			onValueChange?.(null);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	...
	const selectedCountryLabel =
		countries.find(c => String(c.id ?? c.code ?? c.name) === String(selectedCountryId))?.name ?? null;
	const selectedCityLabel = (() => {
		...
		const anyCity = countries
			.flatMap(c => c.cities ?? [])
			.find(ct => String(ct.city_code) === String(selectedCityCode));
			...
			<View style={{flexDirection: 'row', gap: 8}}>
				<View style={{flex: 1}}>
					<TouchableOpacity
						...
						style={styles.input}>
						<Text style={{color: selectedCountryLabel ? colors.grey800 : colors.grey400}}>
							{selectedCountryLabel ?? '국가 선택'}
						</Text>
					</TouchableOpacity>
					{openCountry && (
								...
								{countries.map((c, idx) => {
									...
									return (
										<TouchableOpacity
											...
											<Text style={active ? {color: '#fff'} : {color: 'black'}}>{c.name}</Text>
										</TouchableOpacity>
									);
								})}
							</ScrollView>
				...
				<View style={{flex: 1}}>
					<TouchableOpacity
						...
						style={styles.input}>
						<Text style={{color: selectedCityLabel ? colors.grey800 : colors.grey400}}>
							{selectedCityLabel ?? '도시 선택'}
						</Text>
					</TouchableOpacity>
					{openCity && (
						<View style={styles.dropdown}>
								...
								{citiesForSelectedCountry.map((ct, idx) => {
									...
									return (
										<TouchableOpacity
											...
											style={[styles.optionRow, active ? styles.optionRowActive : undefined]}>
											<Text style={active ? {color: '#fff'} : {color: 'black'}}>{ct.name}</Text>
										</TouchableOpacity>
									);
								})}
							</ScrollView>
