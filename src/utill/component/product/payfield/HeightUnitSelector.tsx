...
export default function HeightUnitSelector({
	cusType,
	options = [],
	label = '키 단위',
	required = false,
	...
	const stored = customMap?.[cusType]?.height_unit ?? null;
	...
	useEffect(() => {
		if (selectedId !== null) {
			dispatch(setCustomField({cusType, fieldId: 'height_unit', value: selectedId}));
			onValueChange?.(selectedId);
		} else {
			dispatch(setCustomField({cusType, fieldId: 'height_unit', value: ''}));
			onValueChange?.(null);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [selectedId, cusType]);
