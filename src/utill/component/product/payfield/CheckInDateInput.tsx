...
export default function CheckInDateInput({cusType = 'send', required = false, onValueChange}: Props) {
	...
	const stored = customMap?.[cusType]?.check_in_date ?? '';
	...
	useEffect(() => {
		if (year && month && day) {
			...
			dispatch(setCustomField({cusType, fieldId: 'check_in_date', value: val}));
			onValueChange?.(val);
		} else {
			dispatch(setCustomField({cusType, fieldId: 'check_in_date', value: ''}));
			onValueChange?.('');
		}
