...
export default function ShoeTypeSelector({
	cusType,
	options = [],
	label = '신발 종류',
	required = false,
	...
	const stored = customMap?.[cusType]?.shoe_type ?? null;
	...
	useEffect(() => {
		if (selectedId !== null) {
			dispatch(setCustomField({cusType, fieldId: 'shoe_type', value: selectedId}));
			onValueChange?.(selectedId);
		} else {
			dispatch(setCustomField({cusType, fieldId: 'shoe_type', value: ''}));
			onValueChange?.(null);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [selectedId, cusType]);
