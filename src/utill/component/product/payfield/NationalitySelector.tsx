...
export default function NationalitySelector({
	...
	const stored = customMap?.[cusType]?.nationality ?? null;
	...
	useEffect(() => {
		if (selectedCode !== null) {
			dispatch(setCustomField({cusType, fieldId: 'nationality', value: selectedCode}));
			onValueChange?.(selectedCode);
		} else {
			// clear
			dispatch(setCustomField({cusType, fieldId: 'nationality', value: ''}));
			onValueChange?.(null);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
