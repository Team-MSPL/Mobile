...
export default function WeightUnitSelector({
	...
	const stored = customMap?.[cusType]?.weight_unit ?? null;
	...
	useEffect(() => {
		if (selectedId !== null) {
			dispatch(setCustomField({cusType, fieldId: 'weight_unit', value: selectedId}));
			onValueChange?.(selectedId);
		} else {
			dispatch(setCustomField({cusType, fieldId: 'weight_unit', value: ''}));
			onValueChange?.(null);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [selectedId, cusType]);
