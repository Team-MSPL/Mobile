...
export default function CheckOutDateInput({cusType = 'send', required = false, onValueChange}: Props) {
	...
	const stored = customMap?.[cusType]?.check_out_date ?? '';
	...
	useEffect(() => {
		if (year && month && day) {
			...
			dispatch(setCustomField({cusType, fieldId: 'check_out_date', value: val}));
			onValueChange?.(val);
		} else {
			dispatch(setCustomField({cusType, fieldId: 'check_out_date', value: ''}));
			onValueChange?.('');
		}
