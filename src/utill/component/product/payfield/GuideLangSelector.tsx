...
import {setGuideLangCode} from '../../../../redux/product/bookingSlice';
...
export default function GuideLangSelector({
	...
	const options: Array<{code?: string; id?: string; name?: string}> =
		rawFields && rawFields.guide_lang && Array.isArray(rawFields.guide_lang.list_option)
			...
			: [];
	const {guideLangCode} = useAppSelector(state => state.bookingSlice);
	...
	const handleSelect = (opt: any) => {
		...
		dispatch(setGuideLangCode(code));
		onSelect?.(code);
		setOpen(false);
	};
	...
	return (
		<View style={{marginTop: 12}}>
			<Text style={{marginBottom: 6, color: colors.grey800, fontSize: 24}}>
				{label}{' '}
				{rawFields?.guide_lang && String(rawFields.guide_lang.is_require).toLowerCase() === 'true' ? (
					<Text style={{color: colors.red400}}>*</Text>
				) : null}
			</Text>
