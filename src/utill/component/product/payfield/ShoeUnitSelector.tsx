...
export default function ShoeUnitSelector({
	cusType,
	options = [],
	label = '신발 단위',
	required = false,
	...
	const stored = customMap?.[cusType]?.shoe_unit ?? null;
	...
	useEffect(() => {
		...
		if (selectedId !== null) {
			dispatch(setCustomField({cusType, fieldId: 'shoe_unit', value: selectedId}));
			onValueChange?.(selectedId);
		} else {
			dispatch(setCustomField({cusType, fieldId: 'shoe_unit', value: ''}));
			onValueChange?.(null);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [selectedId, cusType]);
