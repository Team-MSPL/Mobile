...
export default function CountrySelector({
	...
	return (
		<View style={{marginBottom: 8}}>
			{label ? <Text style={{marginBottom: 6, color: colors.grey800, fontSize: 24}}>{label}</Text> : null}

			<TouchableOpacity activeOpacity={0.85} onPress={() => setOpen(s => !s)} style={styles.countrySelect}>
						...
						{countryOptions.map(opt => {
							...
							return (
								<TouchableOpacity
									...
									<Text style={active ? {color: '#fff'} : {color: 'black'}}>{opt.label}</Text>
								</TouchableOpacity>
							);
						})}
					</ScrollView>
