...
export default function GlassDegreeSelector({
	...
	const stored = customMap?.[cusType]?.glass_degree ?? null;
	...
	useEffect(() => {
		if (selectedVal !== null) {
			dispatch(setCustomField({cusType, fieldId: 'glass_degree', value: selectedVal}));
			onValueChange?.(selectedVal);
		} else {
			dispatch(setCustomField({cusType, fieldId: 'glass_degree', value: ''}));
			onValueChange?.(null);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [selectedVal, cusType]);
						...
						{numericOptions.map(n => {
							...
							return (
								<TouchableOpacity
									...
									<Text style={active ? {color: '#fff'} : {color: 'black'}}>{String(n)}</Text>
								</TouchableOpacity>
							);
						})}
					</ScrollView>
