...
export default function TelCountryCodeSelector({
	...
	const stored = customMap?.[cusType]?.tel_country_code ?? null;
	...
	useEffect(() => {
		...
		if (selectedCode !== null) {
			dispatch(setCustomField({cusType, fieldId: 'tel_country_code', value: String(selectedCode)}));
			onValueChange?.(selectedCode);
		} else {
			dispatch(setCustomField({cusType, fieldId: 'tel_country_code', value: ''}));
			onValueChange?.(null);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
						...
						{options.map((opt, idx) => {
							...
							return (
								<TouchableOpacity
									...
									<Text style={active ? {color: '#fff'} : {color: 'black'}}>
										{labelText} {opt.code ? `(+${opt.code})` : ''}
									</Text>
								</TouchableOpacity>
							);
						})}
